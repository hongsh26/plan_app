...
- **계정 삭제를 통한 `disabled` 차단 회피를 막을지.** `확정 설계 2` §5.3의 상태 전이도는 `disabled → deleted`를 허용하고, §10 7단계는 `deleted` 시점에 `auth_identities` row를 물리 삭제한다. 따라서 운영상 차단된 사용자가 계정을 삭제하고 같은 Apple 계정으로 재가입하면 차단이 풀린다. 막으려면 삭제 후에도 subject의 해시를 보존해 대조해야 하는데, 이는 §4 데이터 최소화 및 §10의 삭제 기대와 정면으로 충돌한다. 실제 남용이 관측되기 전에는 결정하지 않는다. 남용 방지 가치와 개인정보 보존의 트레이드오프이므로 §11(보안)과 함께 판단한다.
