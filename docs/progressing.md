...
`feature/server-skeleton` 브랜치에서 구현하고 독립 리뷰까지 받았다. 리뷰 지적 중 Medium은 전부 닫았다. **`main` 병합만 남았다.**
...
1. **`feature/server-skeleton`을 `main`에 병합한다.** P0는 설계 4~9 전체의 공통 선행 조건이라 기능 브랜치에 남겨두지 않는다.
   - 병합 직후 GitHub Actions에서 `server-ci`가 실제로 도는지 확인한다. 초록불만 보지 말고 로그에서 통합 테스트가 `--- PASS`인지(`--- SKIP`이 아닌지) 확인한다.
   - 그 다음 테스트를 일부러 하나 깨뜨려 빨간불이 나는지 확인한다. 이것까지 해야 M-8이 닫힌다.
